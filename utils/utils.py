...
def build_targets(pred_boxes, pred_conf, pred_cls, target, anchors, num_anchors, num_classes, grid_size, ignore_thres, img_dim):
    ...
    :param target:      参考data/coco_images_sample/labels/train2014下的label定义(中心坐标以及w,h)
                        是Tensor形式的Labels, 维度为[2,50,5], 2为batch_size, 50为检测的最大目标数量, 5表示坐标以及类别。如果不满50行填全0表示。
    ...
    :return:
    """
    nB = target.size(0)                    # labels的batch_size
    nA = num_anchors                       # 预定义的锚点个数=3
    nC = num_classes                       # 分类数目,默认为80
    nG = grid_size                         # 特征图feature map中的cell个数, 如13个
    ...
    nCorrect = 0
    for b in range(nB):
        # 循环查看一个img的label中标注的所有实例
        for t in range(target.shape[1]):
            # 全0表示这一行不是文件中定义的目标, 而是填充为0
            if target[b, t].sum() == 0:
                continue
            nGT += 1
